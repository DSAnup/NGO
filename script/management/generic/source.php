...
$EM = new EntityManagement($Table[$Entity = "Source"]);
...
$EM->InputValidation([
	...
	new HTTP\InputValidation("{$Entity}Color", true),
...
$EM->ListColumn([
	...
	new HTML\UI\Datagrid\Column("{$Entity}" . ($Caption = "Color") . "", "{$Caption}", FIELD_TYPE_COLOR),
...
if(isset($_POST["btnInput"])){
	...
	$EM->InputUIHTML([
		...
		HTML\UI\Field(HTML\UI\Input("{$Entity}" . ($Caption = "Color") . "", $EM->InputWidth(), null, true, INPUT_TYPE_COLOR), "{$Caption}", null, true, $EM->FieldCaptionWidth()),
...
if(isset($_POST["btnExport"])){
	...
	foreach($Recordset as $RecordIndex => $Record)$CSVRow[] = [
		($Field = "#") => $RecordIndex + 1, 
		($Field = "Source Name") => $Record["SourceName"], 
	];
	$Terminal->LetDownload($CSVRow, null, "Source " . date("Y-m-d H-i-s") . " " . rand(0, 9999) . ".csv"); 
}
